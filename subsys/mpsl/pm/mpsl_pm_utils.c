...
static bool m_mram_always_on_requested;
...
static void m_mram_request_handler(void)
{
	...
	{
		if (!m_mram_always_on_requested)
		{
			nrfs_mram_set_latency(MRAM_LATENCY_NOT_ALLOWED, NULL);
			m_mram_always_on_requested = true;
		}
	}
	else
	{
		if (m_mram_always_on_requested)
		{
			...
			m_mram_always_on_requested = false;
			mpsl_pm_power_state_set(false);
		}
	}
}
